// Cloudflare Pages Function: POST /api/hlr
export async function onRequestPost(context) {
  const { request, env } = context;
  const cors = { 'Access-Control-Allow-Origin': '*', 'Content-Type': 'application/json' };
  try {
    const body = await request.json();
    if (!body.phone) return new Response(JSON.stringify({ detail: 'Phone required' }), { status: 400, headers: cors });
    const backendUrl = env.BACKEND_URL || 'https://sosint-api.onrender.com';
    const resp = await fetch(`${backendUrl}/api/hlr`, {
